...
class Plan(BaseModel):
    ...
    steps: List[str] = Field(..., description="A list of tasks for the specialized agents. Each task must start with the agent's name, e.g., 'CommsAgent: ...'.")
...
planner_prompt = ChatPromptTemplate.from_template(
    ...
    Assign each step to a specialist: [CommsAgent] for communication tasks.
...
def specialist_node(state: AgentState):
    ...
    if "CommsAgent" in current_task:
        ...
        result = {"output": f"Task '{current_task}' is a placeholder for another agent."}
